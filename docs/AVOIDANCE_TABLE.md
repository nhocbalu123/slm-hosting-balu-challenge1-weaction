...
| 3   | No API wrapper validation    | FastAPI uses Pydantic schemas for `/v1/chat/completions`, including rejecting empty user messages                                     | invalid request returns 422                            |
| 4   | No authentication            | API checks `X-API-Key` or bearer token with constant-time comparison when `API_KEYS` is set                                           | request without key returns 401                        |
| 5   | No quota or rate limit       | FastAPI has per-key chat quota; Nginx has `limit_req`; health/model-list checks authenticate but do not consume chat quota            | burst test returns 429                                 |
| 6   | Unstructured logs            | FastAPI and Nginx emit JSON logs with latency and request metadata                                                                   | screenshot of `docker logs`                            |
| 7   | Weak health check            | `/v1/health` calls provider `/models` and verifies the configured model ID (`VLLM_MODEL` / `FALLBACK_MODEL`) appears in the list; a running provider without the expected model is reported `healthy: false` | health ok/degraded/down screenshots                    |
| 8   | No timeout or fallback       | Provider client has timeouts, Nginx timeouts cover the primary-plus-fallback budget, and provider request errors avoid fallback       | stop vLLM and show fallback response                   |
